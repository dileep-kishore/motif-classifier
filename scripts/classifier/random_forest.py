...
def run_classifier(feature_path, labelled_path):
    ...
    scores = cross_val_score(model, feature_df , labels_df, cv=ShuffleSplit(n_splits=10, train_size=0.7, random_state=0), verbose=1)
    # scores = cross_val_score(model, feature_df , labels_df, cv=2)
    dummy_scores = cross_val_score(dummy_model, feature_df, labels_df, cv=ShuffleSplit(n_splits=10, train_size=0.7, random_state=0))
    print('scores=',scores)
    ...
    # print('dummy', dummy_model.score(feature_df[500:,:], labels_df[500:]))
